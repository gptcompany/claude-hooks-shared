...
def get_git_changes() -> dict:
    ...
    try:
        ...
        code_files = [
            f for f in all_files if is_code_file(f) and not should_exclude_file(f)
        ]
...
def should_trigger_alpha_debug(stop_reason: str, changes: dict) -> tuple[bool, str]:
    ...
    # HIGH PRIORITY: SpecKit phase completion
    for indicator in SPECKIT_INDICATORS:
        if indicator.lower() in stop_lower:
            return True, f"SpecKit phase completion detected: '{indicator}'"

    # Check stop reason for implementation indicators
    ...
    # Trigger if stop reason indicates implementation work
    if any(kw in stop_lower for kw in implementation_keywords):
        return True, "Implementation phase detected"

    # Trigger if significant code changes exist regardless of reason
    if changes["total_lines"] >= 100:
        return (
            True,
            f"Significant code changes: {changes['total_lines']} lines in {len(code_files)} files",
        )

    # Trigger if multiple code files changed
    if len(code_files) >= 3:
...
def main():
    ...
    # Get stop reason from Claude
    # FIX R4-B2: Sanitize and truncate input to prevent DoS
    stop_reason = input_data.get("stop_reason", input_data.get("reason", ""))
    if not isinstance(stop_reason, str):
        stop_reason = str(stop_reason) if stop_reason else ""
    stop_reason = stop_reason[:MAX_STOP_REASON_LENGTH]
    ...
    if not should_trigger:
        ...
        should_trigger, trigger_reason = should_trigger_alpha_debug(
            stop_reason, changes
        )

...
def load_or_create_bundle(input_data):
    ...
    # Create new bundle
    bundle = {"session_id": get_session_id(input_data), "created_at": datetime.now().isoformat(), "operations": []}
        ...
        if tool_name:
            ...
            if tool_name in ["Read", "Edit", "Write", "MultiEdit"]:
                ...
                if tool_name in ["Edit", "MultiEdit"] and "old_string" in tool_input:
                    tool_details["old_string"] = (
                        tool_input["old_string"][:100] + "..."
                        if len(tool_input["old_string"]) > 100
                        else tool_input["old_string"]
                    )
